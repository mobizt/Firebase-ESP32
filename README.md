...
Able to set multiple stream path which may require more memory and single stream path which swiths between stream monitoring and other set/get/push calls in the same Loop() function or in other CPU core tasks using the sane FirebaseData object.  For multi-tasking, while using the same FirebaseData object, the current Firebase call will force the previous unfinished Firebase call to stop and the normal get/set/push/update/delete calls will have higher prority than stream event call. 
